...
<head>
    ...
    <title>Admin Dashboard - Tools & Tech Stack</title>
...
<body class="antialiased selection:bg-white selection:text-black flex h-screen overflow-hidden">
        ...
        <div class="p-6 md:p-12 lg:p-16 max-w-6xl mx-auto">
            ...
            <div class="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    ...
                    <h1 class="text-3xl md:text-4xl font-bold tracking-tight text-white">Tools & Tech Stack</h1>
                    <p class="text-zinc-400 mt-2">Manage the list of technologies you are proficient in.</p>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-white/[0.02] border-b border-white/10 text-xs uppercase tracking-widest text-zinc-500">
                                <th class="p-6 font-semibold w-2/3">Tool Name</th>
        ...
        <div class="bg-[#141414] border border-white/10 rounded-3xl p-8 w-full max-w-md transform scale-95 transition-transform duration-300 shadow-2xl relative modal-content">
            ...
            <form id="formCreate" onsubmit="handleCreate(event)">
                ...
                <div class="mb-8">
                    ...
                    <input type="text" name="name" placeholder="e.g. React, Laravel, Figma" required class="w-full bg-[#0a0a0a] border border-white/10 rounded-xl px-4 py-3.5 text-white placeholder-zinc-600 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all font-jetbrains text-sm">
                </div>
                <button type="submit" class="w-full px-6 py-3.5 rounded-xl text-sm font-bold bg-white text-black hover:bg-gray-200 transition-all shadow-[0_0_15px_rgba(255,255,255,0.15)]">Save Tool</button>
            </form>
        </div>
    </div>
    ...
    <div id="editModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-opacity duration-300 p-4">
        <div class="bg-[#141414] border border-white/10 rounded-3xl p-8 w-full max-w-md transform scale-95 transition-transform duration-300 shadow-2xl relative modal-content">
            ...
            <form id="formEdit" onsubmit="handleEdit(event)">
                ...
                <div class="mb-8">
                    <label class="block text-sm font-semibold text-zinc-300 mb-2">Tool Name</label>
                    <input type="text" id="editInputName" name="name" required class="w-full bg-[#0a0a0a] border border-white/10 rounded-xl px-4 py-3.5 text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all font-jetbrains text-sm">
                </div>
                <button type="submit" class="w-full px-6 py-3.5 rounded-xl text-sm font-bold bg-blue-600 text-white hover:bg-blue-500 transition-all shadow-[0_0_15px_rgba(37,99,235,0.4)]">Update Tool</button>
            </form>
        </div>
    </div>
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-opacity duration-300 p-4">
        <div id="deleteModalContent" class="bg-[#141414] border border-white/10 rounded-3xl p-8 w-full max-w-md transform scale-95 transition-transform duration-300 shadow-2xl text-center modal-content">
            ...
            <p class="text-zinc-400 text-sm mb-8">You are about to remove <span id="deleteToolName" class="font-bold text-white font-jetbrains"></span> from your tech stack.</p>
