...
<head>
    ...
    <title>Admin Dashboard - CV File</title>
...
<body class="antialiased selection:bg-white selection:text-black flex h-screen overflow-hidden">
        ...
        <div class="p-6 md:p-12 lg:p-16 max-w-6xl mx-auto">
            ...
            <div class="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    ...
                    <h1 class="text-3xl md:text-4xl font-bold tracking-tight text-white">CV Settings</h1>
                    <p class="text-zinc-400 mt-2">Manage the PDF Curriculum Vitae file connected to your website's navbar.</p>
                </div>
            ...
            <div class="bg-[#141414] border border-white/10 rounded-3xl overflow-hidden shadow-2xl">
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody id="cvTableBody" class="divide-y divide-white/5 text-sm">
                            ...
                            <tr id="row-1" class="hover:bg-white/[0.02] transition-colors group">
                                <td class="p-6">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="font-bold text-white text-base">PDF CV</div>
                                    </div>
                                </td>
                                <td class="p-6">
                                    <div class="flex items-center gap-2">
                                        <span class="px-2 py-1 rounded bg-white/5 text-xs text-zinc-400 font-jetbrains border border-white/10">.pdf</span>
                                        <a href="/cv.pdf" target="_blank" class="text-zinc-300 hover:text-blue-400 transition-colors font-medium">
                                            my_cv_2024.pdf
                                        </a>
                                    </div>
                                </td>
                                <td class="p-6">
                                    <div class="flex items-center justify-end gap-3 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                                        <!-- View -->
                                        <button type="button" onclick="openViewModal('my_cv_2024.pdf', '/cv.pdf')" class="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-zinc-400 hover:text-white hover:bg-white/10 transition-colors" title="View Details">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                        </button>
                                        <!-- Edit -->
                                        <button type="button" onclick="openEditModal('my_cv_2024.pdf', 'row-1')" class="w-8 h-8 rounded-full bg-blue-500/10 flex items-center justify-center text-blue-400 hover:text-white hover:bg-blue-500/30 transition-colors" title="Edit/Replace">
    ...
    <div id="createModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-opacity duration-300 p-4">
        <div class="bg-[#141414] border border-white/10 rounded-3xl p-8 w-full max-w-md transform scale-95 transition-transform duration-300 shadow-2xl relative modal-content">
            ...
            <h3 class="text-2xl font-bold text-white mb-2">Upload CV</h3>
            <p class="text-zinc-400 text-sm mb-6">Upload your latest Curriculum Vitae in PDF format.</p>
            ...
            <form id="formCreate" onsubmit="handleCreate(event)" enctype="multipart/form-data">
                ...
                <div class="mb-8">
                    ...
                    <div class="relative group border-2 border-dashed border-white/10 rounded-2xl bg-[#0a0a0a] hover:bg-white/5 hover:border-white/20 transition-all p-8 text-center cursor-pointer flex items-center justify-center min-h-[160px]">
                        ...
                        <input type="file" id="createFileInput" name="cv_file" accept="application/pdf" required class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" onchange="updateFileName(this, 'createFileName')">
    ...
    <div id="editModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-opacity duration-300 p-4">
        <div class="bg-[#141414] border border-white/10 rounded-3xl p-8 w-full max-w-md transform scale-95 transition-transform duration-300 shadow-2xl relative modal-content">
            ...
            <h3 class="text-2xl font-bold text-white mb-2">Replace CV</h3>
            <p class="text-zinc-400 text-sm mb-6">Upload a new PDF to replace the current CV.</p>
            ...
            <form id="formEdit" onsubmit="handleEdit(event)" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <div class="px-4 py-3 bg-[#0a0a0a] border border-white/10 rounded-xl text-zinc-300 font-jetbrains text-sm flex items-center gap-3">
                        ...
                        <span id="currentFileName">my_cv_2024.pdf</span>
                    </div>
                </div>
                ...
                <div class="mb-8">
                    ...
                    <div class="relative group border-2 border-dashed border-white/10 rounded-xl bg-[#0a0a0a] hover:bg-white/5 hover:border-white/20 transition-all p-6 text-center cursor-pointer flex items-center justify-center">
                        <input type="file" id="editFileInput" name="cv_file" accept="application/pdf" required class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" onchange="updateFileName(this, 'editFileName')">
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-opacity duration-300 p-4">
        <div class="bg-[#141414] border border-white/10 rounded-3xl p-8 w-full max-w-md transform scale-95 transition-transform duration-300 shadow-2xl text-center modal-content">
            ...
            <p class="text-zinc-400 text-sm mb-8">You are about to permanently delete this CV file from your server.</p>
    ...
    <script>
        ...
        function handleCreate(e) {
            ...
            swalDark.fire({
                icon: 'success', title: 'CV Uploaded!', text: 'Your PDF CV has been uploaded successfully.',
        ...
        function handleEdit(e) {
            ...
            swalDark.fire({
                icon: 'success', title: 'CV Replaced!', text: 'Your new PDF CV has been updated.',
                showConfirmButton: false, timer: 1500
        ...
        function handleDelete(e) {
            ...
            swalDark.fire({
                icon: 'success', title: 'Deleted!', text: 'The CV file has been removed.',
                showConfirmButton: false, timer: 1500
            }).then(() => {
